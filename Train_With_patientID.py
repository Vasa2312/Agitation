...
def compute_eda_spike_density(eda_signal, fs=FS, window_sec=60):
    ...
    for i in range(len(eda_signal)):
        ...
        density[i] = np.sum((peaks >= start) & (peaks <= i))
...
def load_and_segment(csv_path):
    ...
    # Filter signals
    filtered = df[["eda", "enmo", "bvp", "temp"]].apply(filter_signal, axis=0)

    # Compute spike density
    eda_density = compute_eda_spike_density(filtered["eda"].values, fs=FS)

    # Combine signals
    signals = np.column_stack([filtered.values, eda_density])
    ...
    for w in range(num_windows):
        ...
        if end > num_samples: continue

        window = signals[start:end]
        # Label window as 1 if ANY agitation occurred in this period
...
def preprocess_from_list(participant_dir, agitation_list):
    ...
    with open(agitation_list, "r") as f:
        agitation_days = [line.strip().replace(".mat", ".csv") for line in f]

    for csv_name in agitation_days:
        if not csv_name.startswith(participant_name + "-"): continue
...
if __name__ == "__main__":
    ...
    for idx, participant in enumerate(participants, start=1):
        ...
        X, y = preprocess_from_list(participant_path, agitation_txt)

        if X is not None:
            all_X.append(X)
            all_y.append(y)
            # Create an ID array matching the number of windows (X.shape[0])
            # We use 'idx' as the unique integer for this patient
            p_ids = np.full(X.shape[0], idx)
    ...
    ids_final = np.concatenate(all_pids, axis=0)

    out_dir = Path("processed_17p")
    out_dir.mkdir(exist_ok=True)

    # Saved with names the Training Script expects
    np.save(out_dir / "X_train.npy", X_final)
    np.save(out_dir / "y_train.npy", y_final)
    np.save(out_dir / "pids_train.npy", ids_final) # Critical for baseline logic

    print("\n✅ Preprocessing Complete")
    print(f"Total Windows: {X_final.shape[0]}")
    print(f"Unique Patients: {len(np.unique(ids_final))}")
